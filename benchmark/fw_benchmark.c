 ...
 */
int run_benchmark(const int from_n, const int to_n, const int step) {
    int i;
	...
	for (i = 0; i < num_of_configs; i++) {
        ...
        assert(apsp != NULL);
        clock_t start = clock();
        status = floyd_warshall(m, n, apsp, &err);
        if (status != 0) {
			fprintf(stderr, "Error during floyd warshall execution: %s", err.err_msg);
			return EXIT_FAILURE;
        }
        clock_t end = clock();
...
int main(int argc, char **argv) {
    ...
    return run_benchmark(from_n, to_n, step);
}
